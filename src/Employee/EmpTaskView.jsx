...
//             task.taskId === selectedTask.taskId ? { ...task, statusId, statusAdded: true } : task
//           ));
...
//         setTaskForm({
//           taskName: status.task ? status.task.taskName : '', // Handle possible null task
//           taskStatus: status.taskStatus || 'Not started',
//           estimatedHours: status.totalHours ? status.totalHours.toString() : '0'
//         });
//         setSelectedTask(status.task); // Ensure selectedTask is set
//         setOpen(true);
//       } else {
//         setError('Status not found');
//       }
//     } catch (err) {
//       console.error('Error fetching status for edit:', err.response?.data || err.message || err);
//       setError(`Error fetching status for edit: ${err.response?.data || err.message || 'Unknown error'}`);
//     }
//   };
...
//       return 0;
//     }
//     return tasks.reduce((acc, task) => acc + (Number(task.estimatedHours) || 0), 0);
//   };

//   const totalDays = projectDetails ? calculateTotalDays(projectDetails.startDate, projectDetails.endDate) : 0;
//   const estimatedHours = projectDetails ? calculateEstimatedHours(totalDays) : 0;
...
//           <a className="navbar-brand">Employee Task View</a>
//           <div className="collapse navbar-collapse">
//             <a className="nav-link text-white" href="#" onClick={handleChartOpen}>Chart</a>
//             <a className="nav-link text-white" href="#" onClick={() => window.location.href = "/"}>Logout</a>
//           </div>
//         </div>
//       </nav>
...
//                 </Typography>
//                 <Typography variant="h6" color="textSecondary">
//                   Duration: {projectDetails.startDate} to {projectDetails.endDate}
//                 </Typography>
//                 <Typography variant="h6" color="textSecondary">
...
//                 </Typography>
//                 <Typography variant="h6" color="textSecondary">
//                   Total Hours Assigned: {totalHours}
//                 </Typography>
//               </CardContent>
//             </Card>
//           )}
...
//                     <Typography color="textSecondary">{task.taskStatus || 'Not started'}</Typography>
//                     <Typography color="textSecondary">Estimated Hours: {task.estimatedHours || '0'}</Typography>
//                   </CardContent>
//                   <CardActions>
//                     <IconButton
//                       color="primary"
...
//             <Box style={{ padding: '20px', margin: 'auto', marginTop: '10%', maxWidth: '80%' }}>
//               <TaskComparisonChart />
//             </Box>
//           </Modal>
//         </div>
...
//   const [alertMessage, setAlertMessage] = useState('');
//   const [isAddDisabled, setIsAddDisabled] = useState(false);
//   const [taskStatusAdded, setTaskStatusAdded] = useState(false); // New state for tracking task status
...
//           setAlertMessage(message || 'Status successfully added!');
//           setShowAlert(true);
//           setTaskStatusAdded(true); // Update the state
//           setIsAddDisabled(true); // Disable the Add button after adding
//           handleClose();
//         } else {
//           throw new Error('Unexpected response format');
...
//         const { statusId, message } = response.data;
//         if (typeof statusId === 'number') {
//           // Store the statusId in sessionStorage
//           sessionStorage.setItem('statusId', statusId.toString());
//           setTasks(tasks.map(task =>
//             task.taskId === selectedTask.taskId ? { ...task, statusId, taskStatus: taskForm.taskStatus, estimatedHours: taskForm.estimatedHours } : task
//           ));
//           setAlertMessage(message || 'Status successfully added!');
...
const EmpViewTask = () => {
  ...
  //         const totalHours = tasksWithStatus.reduce((acc, task) => acc + (task.estimatedHours || 0), 0);
  ...
  {tasks.map((task) => (
    <Grid item xs={12} sm={6} md={4} lg={3} key={task.taskId}>
      ...
      <Box style={{ 
        ...
        overflow: 'auto' // Add overflow auto for content overflow handling
      }}>
        <TaskComparisonChart
          calculateEstimatedHours={calcultedestimatedHours}
